...
// ---------------------------------------------------------------------------------------------------------------------
StartupResult
RakPeer::Startup(unsigned int maxConnections, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount,
                 int threadPriority)
{
    if (IsActive())
...
#if !defined(__native_client__)
        int zero=0;
        if (SocketLayer::SendTo(rns, (const char*) &zero,4, rns->GetBoundAddress())!=0)
        {
            DerefAllSockets();
            return SOCKET_FAILED_TEST_SEND;
        }
...
//        return false;
    ...
    return SendConnectionRequest(host, remotePort, passwordData, passwordDataLength, publicKey, connectionSocketIndex,
                                 0, sendConnectionAttemptCount, timeBetweenSendConnectionAttemptsMS, timeoutTime);
}
...
//    RakNet::TimeMS timeout;
    ...
    for (i=0; i < socketList.Size(); i++)
    {
        if (SocketLayer::SendTo(socketList[i], (const char*) &i,1,socketList[i]->GetBoundAddress())!=0)
            break;
    }
    */
    ...
    while ( isRecvFromLoopThreadActive.GetValue()>0 && RakNet::GetTimeMS()<timeout )
    {
        ...
        for (i=0; i < socketList.Size(); i++)
        {
            SocketLayer::SendTo(socketList[i], (const char*) &i,1,socketList[i]->GetBoundAddress());
        }
...
void RakPeer::SendLoopback(const char *data, const int length)
{
    ...
    Packet *packet = AllocPacket(length);
...
//    Packet **threadPacket;
    ...
    for (i=0; i < socketList.Size(); i++)
    {
        while (1)
        {
            ...
            {
                bufferedPackets.Deallocate(recvFromStruct);
                break;
            }
            else
            {
...
// ---------------------------------------------------------------------------------------------------------------------
void
RakPeer::CloseConnection(const AddressOrGUID target, bool sendDisconnectionNotification, unsigned char orderingChannel,
                         PacketPriority disconnectionNotificationPriority)
{
    /*
...
SystemAddress RakPeer::GetSystemAddressFromIndex(unsigned int index)
{
    ...
    if (index < maximumNumberOfPeers)
        if (remoteSystemList[index].isActive && remoteSystemList[index].connectMode ==
...
// ---------------------------------------------------------------------------------------------------------------------
void
RakPeer::GetSystemList(DataStructures::List<SystemAddress> &addresses, DataStructures::List<RakNetGUID> &guids) const
{
    addresses.Clear(false);
    guids.Clear(false);
    ...
    for (i = 0; i < activeSystemListSize; i++)
    {
        ...
        {
            addresses.Push((activeSystemList[i])->systemAddress);
            guids.Push((activeSystemList[i])->guid);
        }
    }
}
...
//        return;
    ...
    SocketLayer::SendTo( socketList[realIndex], (const char*)bitStream.GetData(), (int) bitStream.GetNumberOfBytesUsed(), systemAddress );
    */
                                ...
                                DataStructures::List<RakNetStatistics> &statistics)
{
    addresses.Clear(false);
    guids.Clear(false);
    statistics.Clear(false);

    if (remoteSystemList == 0 || endThreads == true)
        return;

    for (unsigned i = 0; i < activeSystemListSize; i++)
    {
        if ((activeSystemList[i])->isActive && (activeSystemList[i])->connectMode == RakPeer::RemoteSystemStruct::CONNECTED)
        {
            addresses.Push((activeSystemList[i])->systemAddress);
            guids.Push((activeSystemList[i])->guid);
...
unsigned int RakPeer::GetNumberOfRemoteInitiatedConnections(void) const
{
    ...
    unsigned int numberOfIncomingConnections = 0;
    for (unsigned i = 0; i < activeSystemListSize; i++)
    {
        if ((activeSystemList[i])->isActive &&
            (activeSystemList[i])->connectMode == RakPeer::RemoteSystemStruct::CONNECTED &&
...
RakPeer::AssignSystemAddressToRemoteSystemList(const SystemAddress systemAddress, RemoteSystemStruct::ConnectMode connectionMode,
                                               ...
                                               int incomingMTU, RakNetGUID guid, bool useSecurity)
{
    ...
    if (limitConnectionFrequencyFromTheSameIP)
    {
        if (!IsLoopbackAddress(systemAddress, false))
        {
            for (unsigned i = 0; i < maximumNumberOfPeers; i++)
            {
                if (remoteSystemList[i].isActive && remoteSystemList[i].systemAddress.EqualsExcludingPort(systemAddress) &&
    ...
    *thisIPConnectedRecently = false;
    ...
    for (unsigned assignedIndex = 0; assignedIndex < maximumNumberOfPeers; assignedIndex++)
    {
        if (!remoteSystemList[assignedIndex].isActive)
        {
            ...
            for (unsigned j = 0; j < PING_TIMES_ARRAY_SIZE; j++)
            {
            ...
            RakAssert(indexLoopupCheck == (int) assignedIndex);
#endif
            return remoteSystem;
        }
    }
...
{
    return (unsigned int) (SystemAddress::ToInteger(sa) % (maximumNumberOfPeers * REMOTE_SYSTEM_LOOKUP_HASH_MULTIPLE));
}
...
//        RakAssert(remoteSystemList[remoteSystemListIndex].isActive==false);
    ...
    RemoteSystemIndex *rsi = remoteSystemIndexPool.Allocate();
                                      ...
                                      PacketPriority disconnectionNotificationPriority)
{
    ...
    else
    {
        if (performImmediate)
        {
            unsigned index = GetRemoteSystemIndex(target);
            if (index != (unsigned int) -1)
            {
        ...
        {
            BufferedCommandStruct *bcs = bufferedCommands.Allocate();
...
{
    BufferedCommandStruct *bcs = bufferedCommands.Allocate();
    // Making a copy doesn't lose efficiency because I tell the reliability layer to use this allocation for its own copy
    bcs->data = (char *) malloc((size_t) BITS_TO_BYTES(numberOfBitsToSend));
    if (bcs->data == 0)
    {
        RakAssert(0)
                               ...
                               RemoteSystemStruct::ConnectMode connectionMode, uint32_t receipt)
{
    ...
    // Making a copy doesn't lose efficiency because I tell the reliability layer to use this allocation for its own copy
    char *dataAggregate = (char *) malloc((size_t) totalLength);
    if (dataAggregate == 0)
    {
        RakAssert(0)
        return;
    }
    for (unsigned i = 0, lengthOffset = 0; i < numParameters; i++)
    {
        if (lengths[i] > 0)
        {
    ...
    if (priority == IMMEDIATE_PRIORITY)
        quitAndDataEvents.SetEvent(); // Forces pending sends to go out now, rather than waiting to the next update interval
}
                            ...
                            bool useCallerDataAllocation, RakNet::TimeUS currentTime, uint32_t receipt)
{
    ...
    for (unsigned sendListIndex = 0; sendListIndex < sendListSize; sendListIndex++)
    {
        // Send may split the packet and thus deallocate data.  Don't assume data is valid if we use the callerAllocationData
        bool useData = useCallerDataAllocation && !callerDataAllocationUsed && sendListIndex + 1 == sendListSize;
        remoteSystemList[sendList[sendListIndex]].reliabilityLayer.Send(data, numberOfBitsToSend, priority, reliability,
                                                                        orderingChannel, !useData,
                                                                        remoteSystemList[sendList[sendListIndex]].MTUSize,
                                                                        currentTime, receipt);
        if (useData)
...
//            reliability==RELIABLE_SEQUENCED_WITH_ACK_RECEIPT
    ...
    // Return value only meaningful if true was passed for useCallerDataAllocation.
    // Means the reliability layer used that data copy, so the caller should not deallocate it
...
namespace RakNet
{
                                     ...
                                     RakNetSocket2 *rakNetSocket, bool *isOfflineMessage, RakNet::TimeUS timeRead)
    {
        ...
        if (rakPeer->IsBanned(str1))
        {
            ...
            rakNetSocket->Send(&bsp);

            /*
            unsigned i;
            for (i=0; i < rakPeer->pluginListNTS.Size(); i++)
                rakPeer->pluginListNTS[i]->OnDirectSocketSend((char*) bs.GetData(), bs.GetNumberOfBitsUsed(), systemAddress);
            SocketLayer::SendTo( rakNetSocket, (char*) bs.GetData(), bs.GetNumberOfBytesUsed(), systemAddress );
            */
        ...
        if (*isOfflineMessage)
        {
                ...
                length >= sizeof(unsigned char) + sizeof(RakNet::Time) + sizeof(OFFLINE_MESSAGE_DATA_ID))
            {
                    ...
                    rakPeer->AllowIncomingConnections()) // Open connections with players
                {
                    ...
                    // SocketLayer::SendTo( rakNetSocket, (const char*)outBitStream.GetData(), (unsigned int) outBitStream.GetNumberOfBytesUsed(), systemAddress );

                    packet = rakPeer->AllocPacket(sizeof(MessageID));
            ...
            {
                unsigned int dataLength = (unsigned int) (length - sizeof(OFFLINE_MESSAGE_DATA_ID) -
                                                          RakNetGUID::size() - sizeof(MessageID));
            ...
            else if ((unsigned char) (data)[0] == (MessageID) ID_OPEN_CONNECTION_REPLY_1)
            {
                ...
                for (unsigned i = 0; i < rakPeer->requestedConnectionQueue.Size(); i++)
                {
                    ...
                    if (rcs->systemAddress == systemAddress)
                    {
                        ...
                        // SocketLayer::SendTo( rakPeer->socketList[rcs->socketIndex], (const char*) bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), rcs->systemAddress );
                    ...
                    if (rcs->systemAddress == systemAddress)
                    {
                        ...
                        if (remoteSystem == 0)
                            remoteSystem = rakPeer->AssignSystemAddressToRemoteSystemList(systemAddress,
                                                                                          RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER,
                                                                                          rcs->socket ? rcs->socket : rakNetSocket,
                                                                                          &thisIPConnectedRecently, bindingAddress,
                                                                                          mtu, guid, doSecurity);
            ...
            else if ((unsigned char) (data)[0] == ID_OPEN_CONNECTION_REQUEST_1 && length > (int) (1 + sizeof(OFFLINE_MESSAGE_DATA_ID)))
            {
                ...
                // SocketLayer::SendTo( rakNetSocket, (const char*) bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), systemAddress );
            ...
            else if ((unsigned char) (data)[0] == ID_OPEN_CONNECTION_REQUEST_2)
            {
                ...
                if (rakPeer->_using_security)
                {
                    char str1[64];
                    systemAddress.ToString(false, str1);
                    requiresSecurityOfThisClient = !rakPeer->IsInSecurityExceptionList(str1);

                    uint32_t cookie;
                    bs.Read(cookie);
                    CAT_AUDIT_PRINTF("AUDIT: Got cookie %i from %i:%i\n", cookie, systemAddress);
                    if (!rakPeer->_cookie_jar->Verify(&systemAddress.address, sizeof(systemAddress.address), cookie))
                        return true;
                    ...
                    if (requiresSecurityOfThisClient && clientWroteChallenge == 0)
                        return true; // Fail, client doesn't support security, and it is required
                ...
                if (IPAddrInUse & GUIDInUse)
                {
                    ...
                    outcome = 4; // ID_ALREADY_CONNECTED (silently disconnected, restarted rakNet)
                else
                ...
                if (outcome == 1)
                {
                    ...
                    if (requiresSecurityOfThisClient)
                    {
                        ...
                        bsAnswer.WriteAlignedBytes((const unsigned char *) rssFromSA->answer,
                    ...
                    // SocketLayer::SendTo( rakNetSocket, (const char*) bsAnswer.GetData(), bsAnswer.GetNumberOfBytesUsed(), systemAddress );
                ...
                else if (outcome != 0)
                {
                    ...
                    for (unsigned i = 0; i < rakPeer->pluginListNTS.Size(); i++)
                        rakPeer->pluginListNTS[i]->OnDirectSocketSend((const char *) bsOut.GetData(),
                                                                      bsOut.GetNumberOfBitsUsed(), systemAddress);
                    // SocketLayer::SendTo( rakNetSocket, (const char*) bsOut.GetData(), bsOut.GetNumberOfBytesUsed(), systemAddress );
                    RNS2_SendParameters bsp;
                    bsp.data = (char *) bsOut.GetData();
                    bsp.length = bsOut.GetNumberOfBytesUsed();
                ...
                if (!rakPeer->AllowIncomingConnections())
                {
                ...
                bool thisIPConnectedRecently = false;
                rssFromSA = rakPeer->AssignSystemAddressToRemoteSystemList(systemAddress,
                                                                           RakPeer::RemoteSystemStruct::UNVERIFIED_SENDER,
                                                                           rakNetSocket, &thisIPConnectedRecently,
                                                                           bindingAddress, mtu, guid,
                                                                           requiresSecurityOfThisClient);
                ...
                if (requiresSecurityOfThisClient)
                {
                    ...
                    if (rakPeer->_server_handshake->ProcessChallenge(remoteHandshakeChallenge, rssFromSA->answer,
...
// ---------------------------------------------------------------------------------------------------------------------
    ...
    printf("\n");
        ...
        if (remoteSystem && !isOfflineMessage)
        {
            ...
            remoteSystem->reliabilityLayer.HandleSocketReceiveFromConnectedPlayer(data, length, systemAddress,
                                                                                  rakPeer->pluginListNTS, remoteSystem->MTUSize,
...
{
    for (unsigned i = 0; i < socketList.Size(); i++)
    {
...
//    unsigned int socketListIndex;
    ...
    while ((bcs = bufferedCommands.PopInaccurate()) != 0)
    {
        if (bcs->command == BufferedCommandStruct::BCS_SEND)
        {
            ...
            callerDataAllocationUsed = SendImmediate((char *) bcs->data, bcs->numberOfBitsToSend, bcs->priority,
                                                     bcs->reliability, bcs->orderingChannel, bcs->systemIdentifier,
                                                     bcs->broadcast, true, timeNS, bcs->receipt);
        ...
        {
            SocketQueryOutput *sqo = socketQueryOutput.Allocate();
    ...
    if (!requestedConnectionQueue.IsEmpty())
    {
        if (timeNS == 0)
        {
            timeNS = RakNet::GetTimeUS();
        ...
        while (requestedConnectionQueueIndex < requestedConnectionQueue.Size())
        {
            ...
            if (rcs->nextRequestTime < timeMS)
            {
                ...
                bool condition2 = (bool) ((rcs->systemAddress == UNASSIGNED_SYSTEM_ADDRESS) == 1);
                ...
                else
                {
                    ...
                    if (socketToUse->Send(&bsp) == 10040)
                        // if (SocketLayer::SendTo( socketToUse, (const char*) bitStream.GetData(), bitStream.GetNumberOfBytesUsed(), rcs->systemAddress )==-10040)
                    {
    ...
    // remoteSystemList in network thread
    for (unsigned activeSystemListIndex = 0; activeSystemListIndex < activeSystemListSize; ++activeSystemListIndex)
        //for ( remoteSystemIndex = 0; remoteSystemIndex < remoteSystemListSize; ++remoteSystemIndex )
    {
        ...
        remoteSystem->reliabilityLayer.Update(remoteSystem->rakNetSocket, systemAddress, remoteSystem->MTUSize, timeNS,
                                              maxOutgoingBPS, pluginListNTS, &rnr,
        ...
        if (remoteSystem->reliabilityLayer.IsDeadConnection() ||
            ((remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP ||
              remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ASAP_SILENTLY) &&
             !remoteSystem->reliabilityLayer.IsOutgoingDataWaiting()) ||
            (remoteSystem->connectMode == RemoteSystemStruct::DISCONNECT_ON_NO_ACK &&
             (!remoteSystem->reliabilityLayer.AreAcksWaiting() || remoteSystem->reliabilityLayer.AckTimeout(timeMS))) ||
...
//                    memcpy(packet->data+1, undeliveredMessages.GetData(), undeliveredMessages.GetNumberOfBytesUsed());
        ...
        while (bitSize > 0)
        {
            ...
            {
                if ((data)[0] == ID_CONNECTION_REQUEST)
                {
            ...
            else
            {
                                                                             ...
                                                                             sizeof(unsigned short) + sizeof(RakNet::Time) * 2)
                {
                    if (remoteSystem->connectMode == RemoteSystemStruct::HANDLING_CONNECTION_REQUEST)
                    {
                        ...
                        // Bug: If A connects to B through R, A's firstExternalID is set to R. If A tries to send to R,
                        // sends to loopback because R==firstExternalID
                        // Correct fix is to specify in Connect() if target is through a proxy.
                        // However, in practice you have to connect to something else first anyway to know about the proxy.
                        // So setting once only is good enough
                ...
                else if (data[0] == ID_DISCONNECTION_NOTIFICATION)
                {
                ...
                else if ((data)[0] == ID_DETECT_LOST_CONNECTIONS && byteSize == sizeof(unsigned char))
                {
                    // Do nothing
                    free(data);
                }
                else if ((data)[0] == ID_INVALID_PASSWORD)
                {
                    if (remoteSystem->connectMode == RemoteSystemStruct::REQUESTED_CONNECTION)
                    {
                        packet = AllocPacket(byteSize, data);
                ...
                else if ((unsigned char) (data)[0] == ID_CONNECTION_REQUEST_ACCEPTED)
                {
                        ...
                        sizeof(RakNet::Time) * 2)
                    {
                        ...
                        bool alreadyConnected = remoteSystem->connectMode == RemoteSystemStruct::HANDLING_CONNECTION_REQUEST;
                        ...
                        if (allowConnection)
                        {
                            ...
                            free(data); // Ignore, already connected
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if ((data[0] >= (MessageID) ID_TIMESTAMP || data[0] == ID_SND_RECEIPT_ACKED ||
                         data[0] == ID_SND_RECEIPT_LOSS) && remoteSystem->isActive)
                    {
                        packet = AllocPacket(byteSize, data);
                        packet->bitSize = bitSize;
                        packet->systemAddress = systemAddress;
...
{
    if (incomingDatagramEventHandler && !incomingDatagramEventHandler(recvStruct))
        return;
...
void RakPeer::FillIPList(void)
{
    ...
    for (int startingIdx = 0; startingIdx < MAXIMUM_NUMBER_OF_INTERNAL_IDS - 1 && ipList[startingIdx] != UNASSIGNED_SYSTEM_ADDRESS; ++startingIdx)
    {
